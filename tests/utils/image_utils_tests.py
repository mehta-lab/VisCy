...
def test_grid_sample_pixel_values():
    im = np.zeros((15, 20))
    row_ids, col_ids, sample_values = grid_sample_pixel_values(im, grid_spacing=5,)
...
def test_preprocess_image(self):
    ...
    im[:, :5, :, :] = 10
    im_proc = preprocess_image(im, hist_clip_limits=(0, 100),)
...
def test_preprocess_image_norm(self):
    im = np.zeros((5, 10, 15))
    im[:, :5, :] = 10
    im_proc = preprocess_image(im, normalize_im="dataset",)
...
def test_preprocess_image_mask(self):
    im = np.zeros((5, 10, 15))
    im[:, :5, :] = 10
    im_proc = preprocess_image(im, is_mask=True,)
