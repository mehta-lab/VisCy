...
def test_datamodule_caching(preprocessed_hcs_dataset):
    ...
    # Clean up any prior cache to test fresh copy
    ...
    try:
        ...
        # Verify cached store is readable and has same structure
        with open_ome_zarr(cache_path) as cached:
            assert len(list(cached.positions())) == len(
                list(open_ome_zarr(data_path).positions())
            )
