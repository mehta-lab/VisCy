...
class TestUnet25d(unittest.TestCase):
    ...
    def SetUp(self):
        ...
        self.pass_inputs = {'standard': [torch.ones((1,1,5,256,256)), (1,1,1,256,256)],
                            'batch': [torch.ones((3,1,5,256,256)), (3,1,1,256,256)],
                            'multichannel': [torch.ones((1,2,5,256,256)), (1,2,5,256,256)],}
        self.fail_inputs = {'nonsquare': [torch.ones((1,1,5,128,256)), (1,1,1,128,256)],
                            'wrong_dims': [torch.ones((1,1,1,1)), (1,1,1,1)]}
        #possible configurations
        self.configs = {'xy_kernel_size': ((1,1),(3,5),(3,3)),
                        'residual': (True, False),
                        'dropout': (False, 0.25),
                        'num_blocks': (1, 2, 4),
    ...
    def _get_residual_params(self, kwargs, resid_index):
        ...
        try:
            network = Unet25d(in_channels, out_channels, in_depth, out_depth, *kwargs)
            resid_network = Unet25d(in_channels, out_channels, in_depth, out_depth, *resid_kwargs)
