...
class LightningUNet(pl.LightningModule):
    ...
    def __init__(
        ...
        self.log_batches_per_epoch = log_batches_per_epoch  # Set the number of batches to log per epoch
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), lr=self.lr)  # Use the Adam optimizer
        return optimizer
    ...
    def training_step(self, batch: Sample, batch_idx: int):
        ...
        if batch_idx < self.log_batches_per_epoch:
            self.training_step_outputs.extend(self._detach_sample((source, target_one_hot, pred)))
        # Log the training loss
        self.log("loss/train", train_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
    ...
    def validation_step(self, batch: Sample, batch_idx: int):
        ...
        self.log("loss/validate", loss, sync_dist=True, add_dataloader_idx=False, logger=True)
    ...
    def predict_step(self, batch: Sample, batch_idx: int, dataloader_idx: int = 0):
        ...
        return self._predict_pad.inverse(self.forward(source))  # Make a prediction and remove the padding

    # Define what happens at the end of a training epoch
    def on_train_epoch_end(self):
        self._log_samples("train_samples", self.training_step_outputs)  # Log the training samples
    ...
    def _detach_sample(self, imgs: Sequence[Tensor]):
        ...
        return [[np.squeeze(img[i].detach().cpu().numpy().max(axis=1)) for img in imgs] for i in range(self.log_samples_per_batch)]
    ...
    def _log_samples(self, key: str, imgs: Sequence[Sequence[np.ndarray]]):
        ...
        for sample_images in imgs:  # For each sample image
            ...
            for i, image in enumerate(sample_images):  # For each image in the sample images
                ...
                for channel in image:  # For each channel in the image
                    channel = rescale_intensity(channel, out_range=(0, 1))  # Rescale the intensity of the channel
                    render = get_cmap(cm_name)(channel, bytes=True)[..., :3]  # Render the channel
                    images_row.append(render)  # Append the render to the list of image rows
            images_grid.append(np.concatenate(images_row, axis=1))  # Append the concatenated image rows to the list of image grids
