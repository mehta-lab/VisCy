"""Test stage data modules for evaluating segmentation and intensity metrics."""
...
# TODO: potentially merge this and the segmentationgi.py, if so, rename segmentations to metrics?


class IntensityDataset(Dataset):
    """Dataset for intensity metrics evaluation.

    Provides pairs of predicted and target images for intensity comparison.
    """
    ...
    def __getitem__(self, idx: int) -> SegmentationSample:
        ...
        pred = torch.from_numpy(
            pred_img[t, self.pred_channel, self.pred_z_slice].astype(np.float32)
        )
        target = torch.from_numpy(
            target_img[t, self.target_channel, self.target_z_slice].astype(np.float32)
        )
        return {"pred": pred, "target": target, "position_idx": p, "time_idx": t}


class IntensityDataModule(LightningDataModule):
    """Data module for intensity metrics evaluation."""
    ...
    def setup(self, stage: str) -> None:
        ...
        self.test_dataset = IntensityDataset(
            self.pred_dataset,
            self.target_dataset,
            self.pred_channel,
            self.target_channel,
