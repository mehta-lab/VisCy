"""Well and FOV selection utilities for HCS datasets."""
...
def _filter_wells(plate: Plate, include_wells: list[str] | None) -> Generator[Well, None, None]:
    for well_name, well in plate.wells():
        if include_wells is None or well_name in include_wells:
            yield well


def _filter_fovs(well: Well, exclude_fovs: list[str] | None) -> Generator[Position, None, None]:
