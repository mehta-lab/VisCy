"""Memory-mapped dataset and data module for cached OME-Zarr arrays."""
...
class MmappedDataset(Dataset):
    ...
    def __init__(
        ...
        if MemoryMappedTensor is None:
            raise ImportError("tensordict is required for MmappedDataset. Install with: pip install 'viscy-data[mmap]'")
        key = 0
    ...
    def _split_channels(self, volume: Tensor) -> dict[str, Tensor]:
        """Split a volume tensor into per-channel dictionary."""
    ...
    def _preprocess_volume(self, volume: Tensor, norm_meta) -> Tensor:
        """Apply preprocessing transforms to a volume."""
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        if not self._cache_map[idx]:
            ...
            volume = torch.from_numpy(
                position[self.array_key].oindex[time_idx, list(self.channels.values())].astype(np.float32)
            )
...
class MmappedDataModule(GPUTransformDataModule, SelectWell):
    ...
    def cache_dir(self) -> Path:
        ...
        cache_dir = Path(
            ...
            str(torch.distributed.get_rank() if torch.distributed.is_initialized() else 0),
            self.data_path.name,
        )
    ...
    def _buffer_shape(self, arr_shape, fovs) -> tuple[int, ...]:
        """Compute the buffer shape for memory-mapped tensors."""
