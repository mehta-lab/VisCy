"""Triplet sampling data modules for contrastive learning.

Provides :class:`TripletDataset` for sampling anchor/positive/negative
cell patches from tracked OME-Zarr data, and :class:`TripletDataModule`
as the Lightning data module for training triplet-based models.
"""
...
class TripletDataset(Dataset):
    ...
    def __init__(
        ...
        if ts is None:
            raise ImportError(
                "tensorstore is required for TripletDataset. Install with: pip install 'viscy-data[triplet]'"
            )
        self.positions = positions
        self.channel_names = channel_names
        self.channel_indices = [positions[0].get_channel_index(ch) for ch in channel_names]
    ...
    def _sample_negative(self, anchor_row: "pd.Series") -> "pd.Series":
        ...
        if self.time_interval == "any":
            ...
            tracks = self.tracks[self.tracks["t"] == anchor_row["t"] + self.time_interval]
        candidates: "pd.DataFrame" = tracks[(tracks["global_track_id"] != anchor_row["global_track_id"])]
    ...
    def _sample_negatives(self, anchor_rows: "pd.DataFrame") -> "pd.DataFrame":
        """Sample negative examples for each anchor row."""
        negative_samples = [self._sample_negative(row) for _, row in anchor_rows.iterrows()]
...
class TripletDataModule(HCSDataModule):
    ...
    def __init__(
        ...
        reference_pixel_size : float | None, optional
            ...
            When provided, reads the pixel size of the inference dataset from
            its OME-Zarr metadata and computes
        ...
        if reference_pixel_size is not None:
            ...
            self._augmentation_transform = Compose(
                self.normalizations + self.augmentations + [rescale_transform]
            )
            self._no_augmentation_transform = Compose(
                self.normalizations + [rescale_transform]
            )
        else:
    ...
    def _align_tracks_tables_with_positions(
        ...
        """Parse positions in ome-zarr store and assemble tracks tables.
    ...
    def _base_dataset_settings(self) -> dict:
        """Return base dataset settings for TripletDataset."""
        return {
    ...
    def _setup_test(self, *args, **kwargs):
        """Test stage is not supported for self-supervised models."""
