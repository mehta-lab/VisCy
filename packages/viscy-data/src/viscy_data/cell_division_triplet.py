"""Cell division triplet data modules for contrastive learning on npy files.

Provides :class:`CellDivisionTripletDataset` for triplet sampling of cell
division tracks from npy files, and :class:`CellDivisionTripletDataModule`
as the Lightning data module for training cell division models.
"""
...
from viscy_data._typing import DictTransform, TripletSample
...
class CellDivisionTripletDataset(Dataset):
    ...
    def _sample_negative(self, anchor_info: dict) -> Tensor:
        ...
        if not negative_candidates:
            ...
            track = anchor_info["track"]
            anchor_t = anchor_info["timepoint"]
            available_times = [t for t in range(track["num_timepoints"]) if t != anchor_t]
            if available_times:
...
class CellDivisionTripletDataModule(HCSDataModule):
    ...
    def _base_dataset_settings(self) -> dict:
        """Return base dataset settings for CellDivisionTripletDataset."""
        return {
    ...
    def _setup_fit(self, dataset_settings: dict):
        """Set up training and validation cell division triplet datasets."""
        ...
        anchor_transform = (
            no_aug_transform if (self.time_interval == "any" or self.time_interval == 0) else augment_transform
        )
    ...
    def _setup_predict(self, dataset_settings: dict):
        """Set up the prediction cell division triplet dataset."""
