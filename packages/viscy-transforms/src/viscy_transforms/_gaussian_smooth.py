...
__all__ = ["BatchedRandGaussianSmooth", "BatchedRandGaussianSmoothd", "filter3d_separable"]
    ...
    def __init__(
        ...
        for sigma in [sigma_x, sigma_y, sigma_z]:
            if isinstance(sigma, (int, float)):
                ...
                raise ValueError(f"sigma must be float or tuple of 2 values, got {sigma}")
            ...
            for i, (sigma_min, sigma_max, is_range) in enumerate(self.sigma_params):
                if is_range:
                    self._sigma_samples[:, i] = (
                        torch.rand(n_transform, device=data.device) * (sigma_max - sigma_min) + sigma_min
                    )
                else:
