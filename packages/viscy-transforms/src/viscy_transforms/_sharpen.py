"""Batched sharpening transform.

This module provides a GPU-efficient batched sharpening transform for
3D microscopy images using unsharp masking with a Laplacian kernel.
"""
...
__all__ = ["BatchedRandSharpend"]


class BatchedRandSharpend(MapTransform, RandomizableTransform):
    """Randomly apply sharpening to batched 3D microscopy images.

    Uses unsharp masking with a 3D Laplacian kernel to enhance edges and
    fine details in the image. The sharpening strength is controlled by
    alpha blending between the original and sharpened image.
    ...
    alpha_range : tuple[float, float]
        Range for the sharpening strength factor. Samples uniformly between
        min and max values. Higher values produce stronger sharpening.
        Default: (0.1, 0.5).
    prob : float
    ...
    Notes
    -----
    The transform uses a 3x3x3 Laplacian kernel for edge detection.
    The kernel is cached for efficiency across multiple calls.
