...
from viscy_transforms import BatchedRandScaleIntensity, BatchedRandScaleIntensityd
...
def test_batched_rand_scale_intensity(device, prob, factors, channel_wise):
    ...
    transform = BatchedRandScaleIntensity(prob=prob, factors=factors, channel_wise=channel_wise)
    out = transform(img)
    ...
    if prob == 0.0:
        ...
        assert ((scale_factors >= (1 + min_factor)) & (scale_factors <= (1 + max_factor))).all()
    elif prob == 0.5:
...
def test_batched_rand_scale_intensity_dict():
    ...
    transform = BatchedRandScaleIntensityd(keys=["source", "target"], prob=1.0, factors=0.3)
    out = transform(data)
...
def test_batched_scale_intensity_vs_monai(factor_value):
    ...
    batched_transform = BatchedRandScaleIntensity(prob=1.0, factors=(factor_value, factor_value), channel_wise=False)
