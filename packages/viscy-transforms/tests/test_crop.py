...
def test_batched_rand_spatial_crop_random_size_error():
    ...
    with pytest.raises(ValueError, match="Batched transform does not support random size"):
        BatchedRandSpatialCrop(roi_size=[8, 8, 8], max_roi_size=[16, 16, 16], random_size=True)
...
def test_batched_rand_spatial_cropd():
    ...
    transform = BatchedRandSpatialCropd(keys=["image", "mask"], roi_size=roi_size, random_center=True)
...
def test_batched_rand_spatial_cropd_missing_keys():
    ...
    transform = BatchedRandSpatialCropd(keys=["image", "nonexistent"], roi_size=roi_size, allow_missing_keys=True)
...
def test_batched_center_spatial_cropd_missing_keys():
    ...
    transform = BatchedCenterSpatialCropd(keys=["image", "nonexistent"], roi_size=roi_size, allow_missing_keys=True)
