...
from viscy_transforms import BatchedRandAdjustContrast, BatchedRandAdjustContrastd
...
def test_batched_adjust_contrast(device, ndim, prob, compose):
    ...
    changed = ~torch.isclose(result, img, atol=1e-6).all(dim=list(range(1, result.ndim)))
    if prob == 1.0:
        assert changed.all()
    elif prob == 0.5:
        assert changed.any()
...
def test_batched_adjust_contrast_options(gamma, retain_stats, invert_image):
    ...
    transform = BatchedRandAdjustContrast(prob=1.0, gamma=gamma, retain_stats=retain_stats, invert_image=invert_image)
    result = transform(img)
...
def test_batched_adjust_contrast_vs_monai(gamma_value, retain_stats, invert_image):
    ...
    monai_transform = AdjustContrast(gamma=gamma_value, retain_stats=retain_stats, invert_image=invert_image)
