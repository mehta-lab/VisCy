...
from viscy_transforms import BatchedRandGaussianSmooth, BatchedRandGaussianSmoothd
from viscy_transforms._gaussian_smooth import filter3d_separable
...
def test_batched_rand_gaussian_smooth(device, prob):
    ...
    transform = BatchedRandGaussianSmooth(sigma_x=0.5, sigma_y=(1.5, 2.0), sigma_z=0.5, prob=prob)
    out = transform(img)
...
def test_batched_rand_gaussian_smooth_sigma():
    ...
    for sigma_x, sigma_y, sigma_z in [
        ...
        transform = BatchedRandGaussianSmooth(sigma_x=sigma_x, sigma_y=sigma_y, sigma_z=sigma_z, prob=1.0)
        out = transform(img)
        stds.append(out.std())
...
def test_batched_rand_gaussian_smooth_dict():
    ...
    transform = BatchedRandGaussianSmoothd(keys=["a", "b"], sigma_x=0.5, sigma_y=1.0, sigma_z=0.7, prob=1.0)
    out = transform(data)
...
def test_separable_vs_full_3d_equivalence(sigma, kernel_size):
    ...
    output_separable = filter3d_separable(input_tensor, kernel_z, kernel_y, kernel_x, border_type)
