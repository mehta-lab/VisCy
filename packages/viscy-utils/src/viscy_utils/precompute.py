...
def precompute_array(
    ...
    """Precompute normalized images and store as a numpy stack.
    ...
    output_path : Path
        Path to the output numpy stack.
    channel_names : list[str]
        Channel names to normalize.
    subtrahends : list
        Subtrahend for each channel ('mean' or float percentile).
    divisors : list
        Divisor for each channel ('std' or tuple of percentiles).
    ...
    exclude_fovs : list[str] or None, optional
        FOVs to exclude, by default None.
    """
