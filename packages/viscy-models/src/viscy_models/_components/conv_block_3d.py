"""3D convolutional block with configurable layer ordering for UNet architectures."""
...
class ConvBlock3D(nn.Module):
    def __init__(
        ...
        if self.filter_steps == "linear":
            for i in range(self.num_repeats):
                ...
                if self.transpose:
                    self.conv_list.append(nn.ConvTranspose3d(depth_pair[0], depth_pair[1], kernel_size=kernel_size))
                else:
                    self.conv_list.append(nn.Conv3d(depth_pair[0], depth_pair[1], kernel_size=kernel_size))

        elif self.filter_steps == "first":
            if self.transpose:
                for i in range(self.num_repeats):
                    if i == 0:
                        self.conv_list.append(nn.ConvTranspose3d(in_filters, out_filters, kernel_size=kernel_size))
                    else:
                        self.conv_list.append(nn.ConvTranspose3d(out_filters, out_filters, kernel_size=kernel_size))
            else:
                for i in range(self.num_repeats):
                    if i == 0:
                        self.conv_list.append(nn.Conv3d(in_filters, out_filters, kernel_size=kernel_size))
                    else:
                        self.conv_list.append(nn.Conv3d(out_filters, out_filters, kernel_size=kernel_size))
            ...
            if self.transpose:
                for i in range(self.num_repeats):
                    if i == self.num_repeats - 1:
                        ...
                        self.conv_list.append(nn.ConvTranspose3d(in_filters, in_filters, kernel_size=kernel_size))
            else:
                for i in range(self.num_repeats):
                    if i == self.num_repeats - 1:
                        self.conv_list.append(nn.Conv3d(in_filters, out_filters, kernel_size=kernel_size))
                    else:
                        self.conv_list.append(nn.Conv3d(in_filters, in_filters, kernel_size=kernel_size))
    ...
    def model(self):
        """Return parameters as nn.Sequential for optimizer access.
