...
def test_unext2_multichannel(device):
    ...
    model = UNeXt2(
        in_channels=3, out_channels=2, backbone="convnextv2_atto"
    ).to(device)
...
def test_unext2_different_stack_depths(device):
    ...
    model = UNeXt2(
        in_stack_depth=5, out_stack_depth=3, backbone="convnextv2_atto"
    ).to(device)
...
def test_unext2_deconv_decoder(device):
    ...
    model = UNeXt2(
        backbone="convnextv2_atto", decoder_mode="deconv"
    ).to(device)
    x = torch.randn(1, 1, 5, 64, 64, device=device)
    with torch.no_grad():
        out = model(x)
