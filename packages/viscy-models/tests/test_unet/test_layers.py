...
from viscy_models.components import ConvBlock2D, ConvBlock3D
...
class TestConvBlock2D:
    ...
    def test_conv_block_2d_state_dict_keys(self):
        ...
        assert any("batch_norm_0" in k for k in keys), f"Missing batch_norm_0 in {keys}"
    ...
    def test_conv_block_2d_instance_norm(self):
        ...
        assert "instance_norm_0" in named_modules, f"Missing instance_norm_0 in {list(named_modules.keys())}"
...
class TestConvBlock3D:
    ...
    def test_conv_block_3d_dropout_registered(self):
        ...
        named_modules = dict(model.named_modules())
        assert "dropout_0" in named_modules, f"Missing dropout_0 in {list(named_modules.keys())}"
