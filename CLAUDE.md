...
- A step that has not burned CPU **over a window of at least 30 minutes** is never flagged: startup NFS staging is legitimately ~0% CPU, and a short import-time burst must not qualify a step either. Qualification uses cumulative CPU over the step's age when first seen (only if it is already >= 30 min old), or CPU since first sight afterwards (only once >= 30 min have elapsed). A step that once qualified stays qualified through a long idle.
