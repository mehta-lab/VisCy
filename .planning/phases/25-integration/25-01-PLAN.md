...
3. Instantiate MultiExperimentDataModule with:
   ...
   - num_workers=0 (avoid threading issues in test)
...
- ThreadDataLoader with num_workers=0 disables threading. Use num_workers=1 and use_thread_workers=True to match production behavior. Actually, for test stability, set num_workers=0 to avoid deadlocks.
- For the ThreadDataLoader with num_workers, look at how test_datamodule.py handles it. The DataModule default is num_workers=1, but for tests use 0 to avoid threading issues. Actually, ThreadDataLoader doesn't support num_workers=0 the same way as standard DataLoader. Check if passing num_workers=1 is safe in test context. If ThreadDataLoader requires at least 1 worker, use num_workers=1.
...
<task type="auto">
  ...
  <files>applications/dynaclr/examples/configs/multi_experiment_fit.yml</files>
  <action>
