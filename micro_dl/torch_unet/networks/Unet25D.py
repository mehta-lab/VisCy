...
class Unet25d(nn.Module):
    ...
    def __init__(self,
        ...
        :param int in_channels: number of feature channels in (phase, retardance)
        :param int out_channels: number of feature channels out (nuclei, f-actin, etc)
        :param int input_stack_depth: depth of input
        :param int output_stack_depth: depth of output
        ...
        if len(num_filters) != 0:
            assert len(num_filters) == num_blocks, 'Length of num_filters must be equal to num_'\
        ...
        for i in range(num_blocks):
            self.down_conv_blocks.append(ConvBlock3D(forward_filters[i],
                                                     forward_filters[i+1],
        ...
        if self.bottom_block_spatial:
            ...
            self.bottom_transition_block = nn.Conv3d(self.num_filters[-2],
                                                     self.num_filters[-1],
                                                     kernel_size = (1 + in_stack_depth - out_stack_depth,
                                                                    1,
                                                                    1),
        ...
        self.up_conv_blocks = []
        for i in range(num_blocks):
            self.up_conv_blocks.append(ConvBlock3D(backward_filters[i],
                                                   forward_filters[-(i+2)], 
                                                   dropout = self.dropout,
                                                   residual = self.residual,
                                                   activation = activation,
        ...
        for i in range(num_blocks):
            self.skip_conv_layers.append(nn.Conv3d(forward_filters[i+1],
                                                   forward_filters[i+1],
                                                   kernel_size = (1 + in_stack_depth - out_stack_depth,
                                                                  1,
                                                                  1),
