...
class ConvBlock2D(nn.Module):
    def __init__(self, in_filters, out_filters, dropout=False, norm='batch', residual=True, activation='relu',
        ...
        if self.filter_steps == 'linear':
            if self.transpose:
                for i in range(self.num_layers):
                    ...
                    self.conv_list.append(nn.ConvTranspose2d(depth_pair[0], depth_pair[1], kernel_size=kernel_size, padding='same'))
            else:
                for i in range(self.num_layers):
                    depth_pair = (steps[i], steps[i+1]) if i+1 < num_layers else (steps[i],steps[i])
                    self.conv_list.append(nn.Conv2d(depth_pair[0], depth_pair[1], kernel_size=kernel_size, padding='same'))
                    
        elif self.filter_steps == 'first':
            if self.transpose:
                for i in range(self.num_layers):
                    if i == 0:
                        self.conv_list.append(nn.ConvTranspose2d(in_filters, out_filters, kernel_size=kernel_size, padding='same'))
                    else:
                        self.conv_list.append(nn.ConvTranspose2d(out_filters, out_filters, kernel_size=kernel_size, padding='same'))
            else:
                for i in range(self.num_layers):
                    if i == 0:
                        self.conv_list.append(nn.Conv2d(in_filters, out_filters, kernel_size=kernel_size, padding='same'))
                    else:
                        self.conv_list.append(nn.Conv2d(out_filters, out_filters, kernel_size=kernel_size, padding='same'))
        ...
        self.resid_conv = nn.Conv2d(self.in_filters, self.out_filters, kernel_size=1, padding=0)
