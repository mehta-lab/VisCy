...
from utils import dtw_with_matrix, filter_lineages_by_timepoints, identify_lineages
...
def plot_division_events_summary(alignments_data, reference_embeddings, max_plots=20):
        ...
        for lineage_idx, (fov_id, track_ids) in enumerate(all_lineages):
            ...
            if identify_divisions and is_valid and reference_annotations is not None:
                ...
                for ref_idx, query_idx in best_path:
                    if 0 <= ref_idx < len(
                        reference_annotations
                    ) and 0 <= query_idx < len(lineage_annotations):
                        aligned_ref_annotations[query_idx] = reference_annotations[
                            ref_idx
                        ]
                        aligned_query_annotations[ref_idx] = lineage_annotations[
                            query_idx
                        ]
    ...
    if __name__ == "__main__":
        ...
        test_data_timeaware_embeddings = read_embedding_dataset(
            test_data_embedding_path
        )
        ...
        valid_alignments = all_alignment_results[
            all_alignment_results["is_valid"] == True
        ]
        ...
        div_comparison_mask = (
            has_ref_divisions & has_query_divisions & has_any_divisions
        )
...
# %%
# Save the information necessary for aligment
