...
logger.info("| FOV Name | Lineages | Total Tracks | Tracks/Lineage (mean ± std) | Total Timepoints/Lineage (mean ± std) | Timepoints/Track (mean ± std) |")
logger.info("|----------|----------|--------------|------------------------------|---------------------------------------|-------------------------------|")
for _, row in fov_stats.iterrows():
    logger.info(f"| {row['fov_name']} | {row['n_lineages']} | {row['total_tracks']} | "
...
logger.info(f"Tracks per lineage (global): {fov_stats['mean_tracks_per_lineage'].mean():.2f} ± {fov_stats['mean_tracks_per_lineage'].std():.2f}")
logger.info(f"Total timepoints per lineage (global): {fov_stats['mean_total_timepoints'].mean():.2f} ± {fov_stats['mean_total_timepoints'].std():.2f}")
logger.info(f"Timepoints per track (global): {fov_stats['mean_timepoints_per_track'].mean():.2f} ± {fov_stats['mean_timepoints_per_track'].std():.2f}")
...
axes[0, 0].hist(track_stats['total_timepoints'], bins=30, color='#1f77b4', alpha=0.7, edgecolor='black')
axes[0, 0].axvline(track_stats['total_timepoints'].mean(), color='#ff7f0e', linestyle='--', linewidth=2,
...
axes[0, 1].hist(track_stats['n_tracks'], bins=range(1, int(track_stats['n_tracks'].max()) + 2),
                color='#1f77b4', alpha=0.7, edgecolor='black')
axes[0, 1].axvline(track_stats['n_tracks'].mean(), color='#ff7f0e', linestyle='--', linewidth=2,
...
axes[1, 0].hist(track_stats['mean_timepoints_per_track'], bins=30, color='#1f77b4', alpha=0.7, edgecolor='black')
axes[1, 0].axvline(track_stats['mean_timepoints_per_track'].mean(), color='#ff7f0e', linestyle='--', linewidth=2,
...
axes[1, 1].scatter(track_stats['n_tracks'], track_stats['total_timepoints'],
