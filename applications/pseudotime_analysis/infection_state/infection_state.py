...
perturbations_dict = {
    ...
    "zikv": {
        ...
        "features_path": "/hpc/projects/intracellular_dashboard/organelle_dynamics/2025_07_24_A549_SEC61_TOMM20_G3BP1_ZIKV/4-phenotyping/predictions/anndata_predictions/sensor_160patch_104ckpt_ver3max.zarr",
...
consensus_path = None
ALIGN_TYPE = "apoptosis"  # Options: "cell_division" or "infection_state" or "apoptosis"
...
for key in perturbations_dict.keys():
    ...
    features_path = perturbations_dict[key]["features_path"]
...
apoptotic_infected_annotations = [
    {
        "fov_name": "B/2/001000",
        "track_id": [109],
        "timepoints": (26 - n_timepoints_before, 26 + n_timepoints_after + 1),
...
duplicates = alignment_df[
    alignment_df.duplicated(subset=["fov_name", "track_id", "t"], keep=False)
]
if len(duplicates) > 0:
    logger.warning(
        f"Found {len(duplicates)} duplicate (fov_name, track_id, t) combinations!"
    )
...
fov_name_patterns = ["B/2", "consensus"]
