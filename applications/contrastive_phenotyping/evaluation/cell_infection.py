...
cell_uninfected = (
    data[(data["fov_name"] == uninfected_fov) & (data["track_id"] == uninfected_track)][
        ["PHATE1", "PHATE2"]
    ]
...
cell_infected = (
    data[(data["fov_name"] == infected_fov) & (data["track_id"] == infected_track)][
        ["PHATE1", "PHATE2"]
    ]
    .reset_index(drop=True)
    .iloc[::2]
...
plt.savefig(
    "/hpc/projects/comp.micro/infected_cell_imaging/Single_cell_phenotyping/ContrastiveLearning/Figure_panels/arXiv_rev2/infection/Phate_Feb_sensor_infection_phatemap_timeawarentxent_track.png",
    dpi=300,
)
...
pca.fit(
    data.drop(columns=["infection", "fov_name", "track_id", "time", "PHATE1", "PHATE2"])
)
pca_embedding = pca.transform(
    data.drop(columns=["infection", "fov_name", "track_id", "time", "PHATE1", "PHATE2"])
)
...
for time in range(48):
    ...
    for fov_name, track_ids in [
        (fov_name_mock, track_id_mock),
        (fov_name_inf, track_id_inf),
    ]:
        highlight_data = data_test[
            (data_test["time"] == time)
            & (data_test["fov_name"] == fov_name)
            & (
                data_test["track_id"].isin(
                    [item for sublist in track_ids for item in sublist]
                    if isinstance(track_ids[0], list)
                    else track_ids
                )
            )
        ]
        if not highlight_data.empty:
            ...
            ax1.scatter(
                ...
                c=[
                    "steelblue" if pred == 1 else "orange"
                    for pred in highlight_data["infection"]
                ],
    ...
    for fov_name, track_ids in [
        ...
        if not highlight_data.empty:
            ...
            ax1.scatter(
                ...
                s=400,  # Increased size
                c=[
                    "blue" if pred == 1 else "red"
                    for pred in highlight_data["predicted_infection"]
                ],
                alpha=1.0,  # Full opacity for highlighted points
                edgecolor="black",
                linewidth=2,
