...
cm_df = pd.DataFrame(
    cm,
    index=["Background", "Uninfected", "Infected"],
...
plt.scatter(
    pca1[cluster_labels == 0],
    pca2[cluster_labels == 0],
    c=colors[cluster_labels == 0],
    edgecolor="black",
    s=50,
    alpha=0.7,
...
plt.scatter(
    pca1[cluster_labels == 1],
    pca2[cluster_labels == 1],
    c=colors[cluster_labels == 1],
    edgecolor="black",
    s=50,
    alpha=0.7,
    label="Cluster 1 (X)",
...
plt.xlabel("PCA 1")
plt.ylabel("PCA 2")
plt.title("Ground Truth Colors with GMM Cluster Marker Types")
...
plt.scatter(
    umap_df["UMAP1"][cluster_labels == 0],
    umap_df["UMAP2"][cluster_labels == 0],
    c="green",
    edgecolor="black",
    s=50,
    alpha=0.7,
...
plt.scatter(
    umap_df["UMAP1"][cluster_labels == 1],
    umap_df["UMAP2"][cluster_labels == 1],
    c="orange",
    edgecolor="black",
    s=50,
    alpha=0.7,
...
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
plt.title("GMM Clusters in UMAP Space")

plt.legend()
plt.show()

# %% UMAP vis (w/ ground truth colors and GMM cluster markers)
umap_features, umap_projection, umap_df = compute_umap(
    embedding_dataset, normalize_features=True
)
...
# Plot Cluster 0 with circle markers ('o')
plt.scatter(
    umap1[cluster_labels == 0],
    umap2[cluster_labels == 0],
    c=colors[cluster_labels == 0],
    edgecolor="black",
    s=50,
    alpha=0.7,
...
# Plot Cluster 1 with X markers ('x')
plt.scatter(
    umap1[cluster_labels == 1],
    umap2[cluster_labels == 1],
    c=colors[cluster_labels == 1],
    edgecolor="black",
    s=50,
    alpha=0.7,
...
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
plt.title("Ground Truth Colors with GMM Cluster Marker Types in UMAP Space")
