...
def load_annotation(da, path, name, categories: dict | None = None):
    ...
    mi = pd.MultiIndex.from_arrays([da["fov_name"].values, da["id"].values], names=["fov_name", "id"])
...
feb_infection = load_annotation(feb_embedding_dataset, feb_ann_root / "extracted_inf_state.csv", "infection_state", {0.0: "background", 1.0: "uninfected", 2.0: "infected"})
...
X = merged_df.drop(columns=["id", "fov_name", "infection_state"]).values  # Use embeddings as features
...
save_path_csv = "feb_test_regression_predicted_labels_embedding.csv"
