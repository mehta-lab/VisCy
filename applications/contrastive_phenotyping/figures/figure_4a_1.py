...
def plot_umap_infection(features, infection, title):
    ...
    sns.scatterplot(
        x=features["UMAP1"], y=features["UMAP2"], hue=infection, s=7, alpha=0.8
    )
...
mock_cells = feb_features.sel(
    sample=feb_features["fov_name"].str.contains("/A/3")
    | feb_features["fov_name"].str.contains("/B/3")
)
...
plt.figure(figsize=(10, 8))
sns.scatterplot(
    x=feb_features["UMAP1"], y=feb_features["UMAP2"], hue=feb_infection, s=7, alpha=0.8
)
...
sns.histplot(
    x=dengue_cells["UMAP1"],
    y=dengue_cells["UMAP2"],
    bins=50,
    pmax=1,
    cmap="Reds",
    ax=axs[2],
...
sns.histplot(
    x=infected_cells["UMAP1"],
    y=infected_cells["UMAP2"],
    bins=50,
    pmax=1,
    cmap="Reds",
    ax=axs[1, 0],
...
sns.histplot(
    x=uninfected_cells["UMAP1"],
    y=uninfected_cells["UMAP2"],
    bins=50,
    pmax=1,
    cmap="Greens",
    ax=axs[1, 1],
