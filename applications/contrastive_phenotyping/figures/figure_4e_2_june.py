...
def count_infected_cell_states_over_time(embedding_dataset, prediction_df):
    ...
    df = pd.merge(
        df,
        prediction_df[["id", "fov_name", "Infection_Class"]],
        on=["fov_name", "id"],
        how="left",
    )
    ...
    df["well_type"] = df["fov_name"].apply(
        lambda x: (
            "Mock"
            if "/0/1" in x or "/0/2" in x or "/0/3" in x or "/0/4" in x
            else ("Dengue" if "/0/5" in x or "/0/6" in x else "Zika")
        )
    )

    # Group by time, well type, and Predicted_Label to count the number of infected cells
    state_counts = (
        df.groupby(["t", "well_type", "Infection_Class"]).size().unstack(fill_value=0)
    )
...
june_features_path = Path(
    "/hpc/projects/intracellular_dashboard/viral-sensor/infection_classification/models/code_testing_soorya/output/Phase_RFP_smallPatch_June/phaseRFP_36patch_June.zarr"
)
...
state_counts = count_infected_cell_states_over_time(
    june_embedding_dataset, prediction_df
)
