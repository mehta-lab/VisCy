...
def count_infected_cell_states_over_time(embedding_dataset, gmm_df):
    ...
    # Filter by time range (3 HPI to 27 HPI)
    df = df[(df['t'] >= 3) & (df['t'] <= 27)]
    
    # Determine the well type (Mock and Dengue) based on fov_name
    df['well_type'] = df['fov_name'].apply(lambda x: 'Mock' if '/A/3' in x or '/B/3' in x else 'Dengue')
    
    # Group by time, well type, and GMM label to count the number of infected and total cells
    ...
    # Calculate the total number of cells (including uninfected) in each well type
    state_counts['total'] = state_counts.sum(axis=1)
    
    # Calculate the percentage of infected cells
    state_counts['infected_percentage'] = (state_counts['infected'] / state_counts['total']) * 100
...
def plot_infected_cell_states(state_counts):
    ...
    # Loop through each well type (Mock, Dengue)
    for well_type in ['Mock', 'Dengue']:
        ...
        if well_type in state_counts.index.get_level_values('well_type'):
            ...
            if 'infected_percentage' in well_data.columns:
                plt.plot(well_data.index, well_data['infected_percentage'], label=f'{well_type} - Infected')

    plt.title("Percentage of Infected Cells Over Time - Mock and Dengue Wells")
...
gmm_csv_path = "feb_test_regression_predicted_labels_embedding.csv"  # Path to CSV file
