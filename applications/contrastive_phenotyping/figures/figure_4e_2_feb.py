...
def count_infected_cell_states_over_time(embedding_dataset, gmm_df):
    ...
    df = pd.merge(
        df,
        gmm_df[["id", "fov_name", "Predicted_Label"]],
        on=["fov_name", "id"],
        how="left",
    )
    ...
    df["well_type"] = df["fov_name"].apply(
        lambda x: (
            "Mock"
            if "/A/3" in x or "/B/3" in x
            else ("Zika" if "/A/4" in x else "Dengue")
        )
    )

    # Group by time, well type, and GMM label to count the number of infected cells
    state_counts = (
        df.groupby(["t", "well_type", "Predicted_Label"]).size().unstack(fill_value=0)
    )
...
gmm_csv_path = (
    "june_logistic_regression_predicted_labels_feb_pca.csv"  # Path to CSV file
)
