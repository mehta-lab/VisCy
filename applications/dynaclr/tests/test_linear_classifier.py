"""Tests for linear classifier training, prediction, and configuration."""
...
def synthetic_adata_with_unknowns():
    ...
    labels = ["alive"] * 8 + ["dead"] * 8 + ["apoptotic"] * 8 + ["unknown"] * 3 + [np.nan] * 3
...
class TestPredictWithClassifier:
    ...
    def test_predict_adds_uns_classes(self, pipeline_and_adata):
        ...
        assert result.uns["predicted_cell_death_state_classes"] == list(pipeline.classifier.classes_)
...
class TestLoadAndCombineDatasets:
    def test_single_dataset(self, annotated_adata_zarr):
        combined = load_and_combine_datasets([annotated_adata_zarr], task="cell_death_state")
    ...
    def test_filters_unknown_labels(self, tmp_path):
        ...
        ann_df = pd.DataFrame({"fov_name": ["A/1/0"] * n, "id": range(n), "cell_death_state": labels})
        ann_df.to_csv(csv_path, index=False)

        combined = load_and_combine_datasets(
            [{"embeddings": str(zarr_path), "annotations": str(csv_path)}],
            task="cell_death_state",
        )
        assert "unknown" not in combined.obs["cell_death_state"].to_numpy()
    ...
    def test_multiple_datasets(self, annotated_adata_zarr, tmp_path):
        ...
        ann_df = pd.DataFrame({"fov_name": ["B/1/0"] * n, "id": range(n), "cell_death_state": labels})
        ann_df.to_csv(csv_path, index=False)

        dataset2 = {"embeddings": str(zarr_path), "annotations": str(csv_path)}
        combined = load_and_combine_datasets([annotated_adata_zarr, dataset2], task="cell_death_state")
