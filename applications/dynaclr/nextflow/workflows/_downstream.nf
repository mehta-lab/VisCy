// DynaCLR Downstream Sub-workflow
//
// Shared downstream DAG factored out of the evaluation workflow so it can be
// reused from multiple entry points (evaluation, eval_from_embeddings).
// Consumes per-experiment .zarr paths + the parsed JSON manifest and runs
// (reduce / smoothness / mmd / classifiers / plots). Barrier semantics and
// per-experiment-zarr write-order guarantees are identical to the original
// evaluation workflow.
...
workflow DOWNSTREAM {
    take:
        per_exp_zarrs_ch   // channel of per-experiment .zarr path strings
        manifest_ch        // parsed JSON manifest (the map from PREPARE_CONFIGS)
        workspace_dir
