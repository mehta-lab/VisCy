"""Stage 3 phase readout: cosine distance from per-cell pre-baseline.

Same shape as readout_sec61 but uses the quantitative-phase channel
embedding. For claim (a') (DAG §9.2 / discussion §2.1), per-cell phase
onset times are matched against the corresponding fluorescent-marker
onset for Spearman ρ in Stage 4.

Usage::

    cd applications/dynaclr/scripts/pseudotime/3-organelle-remodeling
    uv run python readout_phase.py \
...
def main() -> None:
    """Compute per-cell phase cosine-distance signal and per-FOV mock thresholds."""
    parser = argparse.ArgumentParser(description="Stage 3 phase readout (cosine distance from baseline).")
