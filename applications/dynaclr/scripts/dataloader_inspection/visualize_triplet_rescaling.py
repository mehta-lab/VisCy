"""Visualize scale-aware patch rescaling in TripletDataModule.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--source-channel", required=True, nargs="+",
        help="Channel name(s) to load (e.g. Phase3D).",
    )
    p.add_argument(
        "--z-range", required=True, type=int, nargs=2, metavar=("Z_START", "Z_STOP"),
        help="Z-slice range [start, stop).",
    )
    p.add_argument(
        "--final-yx-patch-size", type=int, nargs=2, default=[224, 224],
        metavar=("Y", "X"),
        help="Target patch size fed to the model (pixels).",
    )
    p.add_argument(
        "--reference-pixel-size", type=float, required=True,
    ...
    p.add_argument(
        "--output", type=Path, default=Path("rescaling_comparison.png"),
...
def main():
    ...
    for i in range(n):
        ...
        ax.set_title(
            ...
            f"{initial_yx[0]}×{initial_yx[1]} px  "
            f"({phys_raw_y:.1f}×{phys_raw_x:.1f} µm)",
            fontsize=9,
        )
        ax.axis("off")
