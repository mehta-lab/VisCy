...
for organelle, config in ORGANELLE_CONFIG.items():
    ...
    for exp in config["experiments"]:
        ...
        aligned = extract_annotation_signal(
            aligned, state_col="organelle_state", positive_value="remodel"
        )
    ...
    # Step 3: Aggregate
    fraction_df = aggregate_population(
        combined, TIME_BINS_MINUTES, signal_type="fraction"
    )
...
for organelle, res in organelle_results.items():
    track_timing = compute_track_timing(
        res["combined_df"], signal_type="fraction"
    )
...
if len(organelle_results) > 1:
    stats_df = run_statistical_tests(
        organelle_results, track_timing_df, control_results or None
    )
    print("\n## Statistical Tests\n")
